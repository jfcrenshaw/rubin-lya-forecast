...
for name in bg_size.keys():
    ...
    if name == "euclid":
        ...
        N_bg_lsst = (1 - A_RATIO_EUCLID) * bg_size[10]
        N_bg_euclid = bg_size["euclid"]
        N_bg = N_bg_lsst + N_bg_euclid  # background galaxies

        N_fg_lsst = (1 - A_RATIO_EUCLID) * fg_size[10]
        N_fg_euclid = fg_size["euclid"]
        N_fg = N_fg_lsst + N_fg_euclid  # foreground galaxies

        # calculate the per-galaxy error on Delta_F
        sigma_dF_lsst = np.log(10) / 2.5 * sigma_du[10]
        sigma_dF_euclid = np.log(10) / 2.5 * sigma_du["euclid"]
        sigma_dF = np.sqrt(
            N_bg
            / (
        ...
        # get the number of galaxies
        N_bg_lsst = (1 - A_RATIO_ROMAN) * bg_size[10]
        N_bg_roman = bg_size["roman"]
        N_bg = N_bg_lsst + N_bg_roman  # background galaxies

        N_fg_lsst = (1 - A_RATIO_ROMAN) * fg_size[10]
        N_fg_roman = fg_size["roman"]
        N_fg = N_fg_lsst + N_fg_roman  # foreground galaxies

        # calculate the per-galaxy error on Delta_F
        sigma_dF_lsst = np.log(10) / 2.5 * sigma_du[10]
        sigma_dF_roman = np.log(10) / 2.5 * sigma_du["roman"]
        sigma_dF = np.sqrt(
            N_bg
            / (N_bg_lsst * sigma_dF_lsst ** (-2) + N_bg_roman * sigma_dF_roman ** (-2))
        )

    elif name == "Y10+euclid+roman":
        # get the number of galaxies
        N_bg_lsst = (1 - A_RATIO_EUCLID - A_RATIO_ROMAN) * bg_size[10]
        N_bg_euclid = bg_size["euclid"]
        N_bg_roman = bg_size["roman"]
        N_bg = N_bg_lsst + N_bg_euclid + N_bg_roman  # background galaxies

        N_fg_lsst = (1 - A_RATIO_EUCLID - A_RATIO_ROMAN) * fg_size[10]
        N_fg_euclid = fg_size["euclid"]
        N_fg_roman = fg_size["roman"]

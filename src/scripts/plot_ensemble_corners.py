...
def corner_plot(filename: str, figsize: tuple, N: int = 10_000) -> sns.PairGrid:
    """Plot the losses saved in the file on the given axis.

    Parameters
    ----------
    filename: str
        The path the the file containing the ensembles.
    figsize: tuple
    ...
    """
    # load the ensemble
    ensemble = FlowEnsemble(file=filename)
...
grid = corner_plot(model_dir / "lsst_ensemble.pzflow.pkl", (5, 1.5))
grid.fig.suptitle("LSST", y=1.1)
grid.savefig(paths.figures / "lsst_corner.pdf")

# save the lsst corner plot
grid = corner_plot(model_dir / "lsst+euclid_ensemble.pzflow.pkl", (7, 1.5))
grid.fig.suptitle("LSST + Euclid", y=1.1)
grid.savefig(paths.figures / "lsst+euclid_corner.pdf")

# save the lsst corner plot
grid = corner_plot(model_dir / "lsst+roman_ensemble.pzflow.pkl", (7, 1.5))

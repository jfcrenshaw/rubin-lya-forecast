...
\begin{figure}[t!]
    \script{calculate_photoz_metrics.py}
    \begin{centering}
        \includegraphics{figures/bg_photoz_metrics.pdf}
        \caption{
            \note{Write this!}
        }
        \label{fig:bg_photoz_metrics}
    \end{centering}
\end{figure}
...
We use $M=\variable{output/M.txt}$ and $N=\variable{output/M.txt}$.

The results of the redshift cut are shown in Figure \ref{fig:bg_photoz_metrics}.
...
We find that, despite our low completeness, we still have very large galaxy samples due to the immense size of the LSST catalog.

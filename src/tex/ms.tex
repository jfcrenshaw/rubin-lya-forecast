...
We will not simulate variations in the \lya signal due to large scale structure, because these variations are subdominant to the uncertainties in estimating $\du$, and will therefore not impact our signal-to-noise estimate in this section.
...
Depending on the context, $m$/$m_\alpha$ might refer to only the LSST $grizy$ bands, or might include the Euclid $YJH$ or Roman $YJHF$ bands.
...
As such, we simulate errors for LSST survey durations from 1-10 years to study how the SNR changes over time.
...
We train an ensemble of 10 normalizing flows using PZFlow \citep{pzflow} to model the conditional distribution $p(z, u | m_\alpha)$.
We train the flows on $100\,000$ randomly selected galaxies from the truth catalog.
We add the expected decrements to the $g$ band (see Figure \ref{fig:decrements}), so that the training set is representative of $\{z, u m_\alpha\}$.
...
\begin{align}
    \int_{2.36}^\infty dz ~ p(z|\hat{m}_\alpha) > 0.95.
...
\begin{align}
    _{i=1}^{~~ M} \{ m_\alpha^i \, | \, m_\alpha^i \sim p(m_\alpha | \hat{m}_\alpha) \},
\end{align}
...
Despite the low completeness, we still have very large galaxy samples due to the immense size of the LSST catalog.
...
\begin{align}
    \mathcal{L}(\du) = \int du \, p(\hat{u}_\alpha | u, \du) \, p(u | \hat{m}_\alpha, z > 2.36).
\end{align}

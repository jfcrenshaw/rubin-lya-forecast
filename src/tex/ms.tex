...
\end{align}
for each galaxy.
We can then directly calculate $p(\hat{u}_\alpha | u, \du) = p(\hat{u}_\alpha | u_\alpha = u + \du)$ using the photometric error model\footnote{\note{I need to note how this is slightly different for the perfect catalog.}}.
...
Once we have a likelihood for each galaxy in the sample, we construct a joint likelihood by taking the product of the individual likelihoods\footnote{A small number of the likelihoods were very noisy -- we removed these from the analysis.}.
We calculate the variance of the joint likelihood and multiply by the square root of the number of galaxies in the sample.
This provides an estimate of the effective Gaussian uncertainty per galaxy, $\sigma_{\Delta u}$.

...
This catalog contains \note{N} galaxies up to redshift \note{N}, with optical LSST photometry ($ugrizy$) and near-infrared (NIR) photometry from Euclid ($YJH$) and the Roman Space Telescope ($YJHF$).
This constitutes the truth catalog.

Throughout the rest of this paper, we will denote true galaxy redshifts with the variable $z$\footnote{Occasionally, $z$ will denote the LSST $z$ band, but this will be obvious from context.}, true $u$ band magnitudes with $u$, and $u$ band magnitudes including \lya extinction with the variable $u_\alpha$.
We will use the variable $m$ to refer to the vector of all other magnitudes.
$m$ always includes the LSST $grizy$ bands, and, depending on the context, might include either the Euclid or Roman bands \note{Note: change this language if I determine that Euclid and Roman will have overlap}.
...
\end{align}
where $b_g \approx 2$ is the linear galaxy bias \needscite, and $\beta_g = \frac{1}{b_g}$ sets the amplitude of RSDs.

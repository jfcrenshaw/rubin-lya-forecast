...
\note{
    To give a sense for the scale of these decrements, we plot the expected photometric error as a function of redshift for the $u$ and $g$ bands (the errors were calculated using the \citep{ivezic2019} error model, assuming 10 years of observation; see Section \ref{sec:catalog}).
}

To give a sense of the significance of these decrements, we plot the expected photometric error for each band as a function of 

\note{
    Mention the plot here.
    The left side shows $u$ band decrements for different values of $alpha$.
    The results are relatively insensitive to this choice, so for now, we use $\alpha=0$.
    The right hand side shows the expected decrements for the 3 bluest bands, as a function of redshift.
    To get an idea of the scale, we plot the expected photometric errors for these bands as a function of redshift (Y1 or Y10?)
}

The question of this paper is whether this decrease in flux can be detected with the Rubin observatory and what cosmological signal can be extracted from it.
\note{I need to state the $u$ band focus here.}

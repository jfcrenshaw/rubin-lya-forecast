...
In this section, we simulate Rubin observations to calibrate the expected SNR of the photometric \lya signal for individual galaxies.
...
We use the simulated galaxy catalog of \citet{graham2018, graham2020}, which is based on the Millennium simulation \citep{springel2005}, including the galaxy formation models of \citep{gonzalez-perez2014} and lightcones from \citep{merson2013}.
...
We then simulate photometric errors according to the \citet{ivezic2019} error model, including the low SNR generalization presented in \needscite and the extension to Euclid and Roman presented in \citet{graham2020}.
...
We train an ensemble of normalizing flows to model the conditional distribution $p(z, u | m_\alpha)$ \needscite.
...
Now that we have trained the model, we wish to select galaxies at $z > 2.36$ whose $u$ bands are maximally impacted by \lya scattering.
